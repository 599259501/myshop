...
elseif ($_REQUEST['act'] == 'toggle_show')
{
  ...
  $sql='UPDATE '.$ecs->table('goods').' SET is_display='.$val.' WHERE goods_id='.$goods_id;
  $result=$db->query($sql);
  if($result){
    clear_cache_files();
...
elseif($_REQUEST['act'] == 'setting'){
    ...
    $good=$db->getRow($sql);
    if($good){
        $sql='INSERT INTO '.$ecs->table('ad_flash').'(img_src,link_src,is_avaliable,create_time,goods_id) VALUES(\''.$good['original_img'].'\',\''.
        '/goods.php?id='.$goods_id.'\',1,\''.date('Y-m-d H:i:s',time()).'\','.$goods_id.')';
        $result=$db->query($sql);
    }
    $res->meta->code=empty($result)?200:10001;
    $res->meta->message=empty($result)?'设置成功':'设置过程中出现异常';
