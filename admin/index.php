...
elseif($_REQUEST['act']=='setting'){
  ...
  $sql='SELECT * FROM '.$ecs->table('goods').' WHERE goods_id='.$good_id;
  $data=$db->getRow($sql);
  if($data){
    $sql='INSERT INTO '.$ecs->table('ad_flash').'(img_src,link_src,is_avaliable,create_time) VALUES(\''.$data['goods_img'].'\',
    \'/shop/goods.php?id='.$good_id.'\',1,\''.date('Y-m-d H:i:s',time()).'\'';
    $result=$db->query($sql);
  }
  $code=$result?200:10001;
  make_json_result();
}
/**
 * license check
