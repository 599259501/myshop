...
function available_shipping_list($region_id_list)
{
    $sql = 'SELECT s.shipping_id, s.shipping_code, s.shipping_name, ' .
            ...
            'FROM ' . $GLOBALS['ecs']->table('shipping') . ' AS s where s.enabled = 1 ORDER BY s.shipping_order';
	
	$shipping_list=$GLOBALS['db']->getAll($sql);
	if(empty($shipping_list)){
		$shipping_list=array(
			array('shipping_name'=>'顺丰快递','shipping_desc'=>'江、浙、沪地区首重为15元/KG，其他地区20元/KG， 续重均为5-6元/KG， 云南地区为8元','format_shipping_fee'=>15,'free_money'=>0),
			array('shipping_name'=>'申通快递','shipping_desc'=>'江、浙、沪地区首重为15元/KG，其他地区18元/KG， 续重均为5-6元/KG， 云南地区为8元','format_shipping_fee'=>15,'free_money'=>0)
		);
	}
...
function get_consignee($user_id)
{
    ...
    else
    {
        ...
        }else{//获取用户未登录的时候所需要的配送信息
			$arr=array(
				array('shipping_name'=>'申通快递','shipping_desc'=>'江、浙、沪地区首重为15元/KG，其他地区18元/KG， 续重均为5-6元/KG， 云南地区为8元','format_shipping_fee'=>15,'free_money'=>0),
				array('shipping_name'=>'顺丰快递','shipping_desc'=>'江、浙、沪地区首重为15元/KG，其他地区20元/KG， 续重均为8-9元/KG， 云南地区为12元','format_shipping_fee'=>20,'free_money'=>0),
			);
		}
