...
function assign_articles($id, $num)
{
    ...
    $cat['url']      = build_uri('article_cat', array('acid' => $id), $cat['name']);//common(f)
...
function assign_template($ctype = '', $catlist = array())
{
    ...
    $smarty->assign('first_catory',        getCatoryChildrenParent(0));  //选取顶级分类
...
/**获取全部父类商品分类信息**/
function getCatoryChildrenParent($id){
	$sql='SELECT * FROM '.$GLOBALS['ecs']->table('category').' WHERE parent_id='.intval($id).' and is_show=1 and parent_id=0 limit 8';
	$res = $GLOBALS['db']->getAll($sql);
	return $res;
}
