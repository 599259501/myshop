...
class integrate
{
    ...
    function login($username, $password, $remember = null)
    {
        ...
        {
            if($this->redis->hExists('user',md5($username)))
            {
                $muser=$this->redis->hget('user',md5($username));
                $muser=(array)json_decode($muser);
                if($muser['password']==$this->compile_password(array('password'=>$password)))
                {
                    if ($this->need_sync)
                    {
            ...
            }else
            {
                if ($this->check_user($username, $password) > 0)
                {
                    ...
                    $this->set_session($username);
                    $this->set_cookie($username, $remember);
                    $this->redis->hSet('user',md5($username),json_encode(array('password'=>$this->compile_password(array('password'=>$password)),'user_name'=>$username)));
                    return true;
                }
                else
                {
